...
class Encoder(nn.Module):
    ...
    def forward(self, cxr_image, enh_image):
        ...
        image_features = F.adaptive_avg_pool2d(image_features, (1, 1)).squeeze(2).squeeze(2)  
        lidar_features = F.adaptive_avg_pool2d(lidar_features, (1, 1)).squeeze(2).squeeze(2)  
...
class ms_transfuser_late(nn.Module):
    '''
    ...
    def __init__(self, args, num_classes, embed_dim): #config,
        ...
        self.mlp_head_cxr = ClassificationHead(num_classes, embed_dim)
        self.mlp_head_enh = ClassificationHead(num_classes, embed_dim)
